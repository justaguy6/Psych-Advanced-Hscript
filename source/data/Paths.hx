...
class Paths
{
	...
	{
		if (OpenFlAssets.exists(getPath(key, type)) || FileSystem.exists(getPath(key, type)))
		{
			return true;
		}
		return false;
	...
	public static function returnGraphic(key:String, ?library:String)
	{
		...
		// trace(path);
		if (OpenFlAssets.exists(path, IMAGE) || FileSystem.exists(path))
		{
		...
		trace('ASSET NOT FOUND, PATH: $path');
		return null;
	}
